...
function notifIcon(type) {
  ...
  for (const [k, v] of Object.entries(TYPE_ICONS)) {
    if (key.includes(k.replace('_', ''))) return v;
  }
      ...
      >
        ...
        {todayList.length > 0 ? (
          <>
            ...
            {todayList.map((n) => <NotifRow key={n.id} n={n} accentColor={colors.primary} />)}
          </>
        ) : null}
        ...
        {earlierList.length > 0 ? (
          <>
            ...
            {earlierList.map((n) => <NotifRow key={n.id} n={n} accentColor={colors.primary} />)}
          </>
        ) : null}
      </ScrollView>
    </View>
...
function NotifRow({ n, accentColor }) {
  ...
  return (
    <View style={[styles.notifRow, !n.read && styles.notifRowUnread]}>
      ...
      {!n.read ? <View style={[styles.unreadDot, { backgroundColor: accentColor }]} /> : null}
    </View>
  );
}
