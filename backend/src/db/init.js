...
async function ensureColumnPostgres(db, table, column, pgType) {
  ...
  if (!row) {
    await db.exec(`ALTER TABLE ${table} ADD COLUMN ${column} ${pgType}`);
  }
}
...
async function initPostgres(db) {
  ...
  // Forward-compatible column adds (same as legacy SQLite init migrations)
  await ensureColumnPostgres(db, 'task_completions', 'evidence_data', 'TEXT');
  await ensureColumnPostgres(db, 'task_completions', 'evidence_mime', 'TEXT');
  await ensureColumnPostgres(
    db,
    'task_completions',
    'evidence_type',
    "TEXT CHECK (evidence_type IN ('Photo','Video'))"
  );
  await ensureColumnPostgres(db, 'task_completions', 'evidence_note', 'TEXT');
  await ensureColumnPostgres(db, 'task_completions', 'parent_note', 'TEXT');
  await ensureColumnPostgres(
    db,
    'task_completions',
    'ai_recommendation',
    "TEXT CHECK (ai_recommendation IN ('Approve','Reject','NeedsParentReview'))"
  );
  await ensureColumnPostgres(db, 'task_completions', 'ai_confidence', 'DOUBLE PRECISION');
  await ensureColumnPostgres(db, 'task_completions', 'ai_reason', 'TEXT');
  await ensureColumnPostgres(db, 'task_completions', 'ai_model', 'TEXT');
  await ensureColumnPostgres(db, 'task_completions', 'ai_analyzed_at', 'TEXT');
  await ensureColumnPostgres(
    db,
    'task_completions',
    'ai_status',
    "TEXT CHECK (ai_status IN ('Completed','Unavailable','Error'))"
  );
  await ensureColumnPostgres(db, 'task_completions', 'disputed', 'INTEGER NOT NULL DEFAULT 0');
  await ensureColumnPostgres(db, 'task_completions', 'dispute_note', 'TEXT');
  await ensureColumnPostgres(db, 'task_completions', 'disputed_at', 'TEXT');
  await ensureColumnPostgres(db, 'task_completions', 'resolved_at', 'TEXT');
  await ensureColumnPostgres(db, 'child_profiles', 'email', 'TEXT');
  await ensureColumnPostgres(db, 'child_profiles', 'password_hash', 'TEXT');
  await ensureColumnPostgres(db, 'child_profiles', 'pin_hash', 'TEXT');
  await ensureColumnPostgres(
    db,
    'child_profiles',
    'giftcard_points_balance',
    'INTEGER NOT NULL DEFAULT 0 CHECK (giftcard_points_balance >= 0 AND giftcard_points_balance <= 100000)'
  );
  await ensureColumnPostgres(
    db,
    'parent_accounts',
    'gp_balance',
    'INTEGER NOT NULL DEFAULT 0 CHECK (gp_balance >= 0 AND gp_balance <= 1000000)'
  );
  await ensureColumnPostgres(db, 'parent_accounts', 'notification_preferences', 'TEXT');
  await ensureColumnPostgres(
    db,
    'tasks',
    'gp_points',
    'INTEGER NOT NULL DEFAULT 0 CHECK (gp_points >= 0 AND gp_points <= 1000)'
  );
  await ensureColumnPostgres(db, 'tasks', 'category', "TEXT NOT NULL DEFAULT 'other'");
  await ensureColumnPostgres(db, 'tasks', 'recurrence_days', 'TEXT');
  await ensureColumnPostgres(
    db,
    'tasks',
    'required_evidence_type',
    "TEXT CHECK (required_evidence_type IS NULL OR required_evidence_type IN ('Photo','Video'))"
  );
  await ensureColumnPostgres(
    db,
    'rewards',
    'points_type',
    "TEXT NOT NULL DEFAULT 'RP' CHECK (points_type IN ('RP','GP'))"
  );
  await ensureColumnPostgres(
    db,
    'redemptions',
  ...
  await ensureColumnPostgres(
    ...
    'points_transactions',
    'points_kind',
    "TEXT NOT NULL DEFAULT 'RP' CHECK (points_kind IN ('RP','GP'))"
  );
  await ensureColumnPostgres(db, 'gaming_sessions', 'denial_code', 'TEXT');
  await ensureColumnPostgres(db, 'child_profiles', 'avatar_url', 'TEXT');
...
async function initSqlite(db) {
  ...
  await ensureColumnSqlite(db, 'task_completions', 'evidence_data', 'evidence_data TEXT');
  await ensureColumnSqlite(db, 'task_completions', 'evidence_mime', 'evidence_mime TEXT');
  await ensureColumnSqlite(db, 'task_completions', 'evidence_type', "evidence_type TEXT CHECK(evidence_type IN ('Photo','Video'))");
  await ensureColumnSqlite(db, 'task_completions', 'evidence_note', 'evidence_note TEXT');
  await ensureColumnSqlite(db, 'task_completions', 'parent_note', 'parent_note TEXT');
  await ensureColumnSqlite(db, 'task_completions', 'ai_recommendation', "ai_recommendation TEXT CHECK(ai_recommendation IN ('Approve','Reject','NeedsParentReview'))");
  await ensureColumnSqlite(db, 'task_completions', 'ai_confidence', 'ai_confidence REAL');
  await ensureColumnSqlite(db, 'task_completions', 'ai_reason', 'ai_reason TEXT');
  await ensureColumnSqlite(db, 'task_completions', 'ai_model', 'ai_model TEXT');
  await ensureColumnSqlite(db, 'task_completions', 'ai_analyzed_at', 'ai_analyzed_at TEXT');
  await ensureColumnSqlite(db, 'task_completions', 'ai_status', "ai_status TEXT CHECK(ai_status IN ('Completed','Unavailable','Error'))");
  await ensureColumnSqlite(db, 'task_completions', 'disputed', 'disputed INTEGER NOT NULL DEFAULT 0');
  await ensureColumnSqlite(db, 'task_completions', 'dispute_note', 'dispute_note TEXT');
  await ensureColumnSqlite(db, 'task_completions', 'disputed_at', 'disputed_at TEXT');
  await ensureColumnSqlite(db, 'task_completions', 'resolved_at', 'resolved_at TEXT');
  await ensureColumnSqlite(db, 'child_profiles', 'email', 'email TEXT');
  await ensureColumnSqlite(db, 'child_profiles', 'password_hash', 'password_hash TEXT');
  await ensureColumnSqlite(db, 'child_profiles', 'pin_hash', 'pin_hash TEXT');
  ...
  await ensureColumnSqlite(db, 'rewards', 'points_type', "points_type TEXT NOT NULL DEFAULT 'RP' CHECK(points_type IN ('RP','GP'))");
  await ensureColumnSqlite(db, 'redemptions', 'points_type', "points_type TEXT NOT NULL DEFAULT 'RP' CHECK(points_type IN ('RP','GP'))");
  await ensureColumnSqlite(db, 'points_transactions', 'points_kind', "points_kind TEXT NOT NULL DEFAULT 'RP' CHECK(points_kind IN ('RP','GP'))");
  await ensureColumnSqlite(db, 'gaming_sessions', 'denial_code', 'denial_code TEXT');
  await db.exec(
  ...
  await ensureColumnSqlite(db, 'child_profiles', 'avatar_url', 'avatar_url TEXT');
  ...
  await ensureColumnSqlite(db, 'rewards', 'giftcard_brand', 'giftcard_brand TEXT');
  await ensureColumnSqlite(db, 'rewards', 'giftcard_denomination_cents', 'giftcard_denomination_cents INTEGER');
