...
/** Serialize child evidence submits so double-invocation cannot race the server. */
let evidenceCompleteSubmitInFlight = false;
...
export async function apiRequest<T = unknown>(
  ...
  const isEvidenceComplete =
    method === "POST" && path.replace(/\/$/, "") === "/tasks/complete";
  if (isEvidenceComplete) {
    if (evidenceCompleteSubmitInFlight) {
    ...
    evidenceCompleteSubmitInFlight = true;
  }
  ...
  try {
    ...
    if (isEvidenceComplete) {
      evidenceCompleteSubmitInFlight = false;
    }
  }
}
